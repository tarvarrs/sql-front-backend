...
class AchievementRepository:
    ...
    async def check_and_award_achievements(
        self, user_id: int, task_tags: list[str]
    ) -> list[dict]:
        if not task_tags:
            return []
        req = select(Achievement).where(
            Achievement.tag.in_(task_tags), Achievement.required_count.is_not(None)
        )
        ...
        for ach in achievements:
            ...
            upsert_req = (
                ...
                .values(
                    user_id=user_id, achievement_id=ach.achievement_id, current_count=1
                )
                .on_conflict_do_update(
                    index_elements=["user_id", "achievement_id"],
                    set_={"current_count": UserAchievementProgress.current_count + 1},
                )
